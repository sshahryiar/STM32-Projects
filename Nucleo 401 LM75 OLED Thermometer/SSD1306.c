...
void OLED_write(unsigned char value, unsigned char control_byte)
{
     ...
     I2C1_Start();
     I2C1_Write(SSD1306_I2C_Address, data_value, 2, END_MODE_STOP);
}
...
void OLED_fill(unsigned char bmp_data)
{
    ...
    unsigned char data_value[(x_max + 1)];
    
    data_value[0] = DAT;
    
    for(x_value = 1; x_value < (x_max + 1); x_value++)
    {
    ...
    for(page = 0; page < y_max; page++)
    {
        ...
        I2C1_Start();
        I2C1_Write(SSD1306_I2C_Address, data_value, (1 + x_max), END_MODE_STOP);
    }
}
